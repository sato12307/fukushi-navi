...
const GENERATED = /^(articles\/toei-|shogai-kojo\/|kawasaki\/|shizuoka\/|yokohama\/|kobe\/)/
...
const files = [
  ...
  //   ★2026-09-17(5) 政令市が4つになった（神戸を追加）。売り場カードのある無料ページは全部見る。
  //     /<市>/kanryo/ は購入者だけが来る画面なので見ない。
  ...['kawasaki', 'shizuoka', 'yokohama', 'kobe'].map((k) => k + '/index.html').filter((f) => fs.existsSync(f)),
].sort()
