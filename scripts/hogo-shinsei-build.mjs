...
#jt thead th { position: sticky; top: 0; z-index: 1; background: var(--soft); cursor: pointer; white-space: nowrap; font-size: .82rem; box-shadow: 0 1px 0 var(--line); }
...
${hist.map((h) => `      <tr><th>${WAREKI(h.y)}（${h.y}）</th><td class="n">${num(h.shinsei)}</td><td class="n">${num(h.torisage)}</td><td class="n">${p2(h.tr)}%</td><td class="n">${num(h.kyakka)}</td><td class="n"><strong>${p2(h.kr)}%</strong></td></tr>`).join('\n')}
  ...
  <table id="jt">
    ...
    <thead><tr>
      ...
      <th data-k="o" class="n">保護開始<br>世帯数</th>
      <th data-k="sp" class="n">申請<br>(人口千対)</th>
      <th data-k="hr" class="n">保護率<br>(人口千対)</th>
      <th data-k="m" class="n">年度末<br>未処理</th>
    </tr></thead>
    <tbody id="jtb"></tbody>
  </table>
  </div>
...
${rankRows(trHi.slice(0, 12), 'tr')}
  ...
  <h2>⑥ 却下されたときにできること</h2>
  <ul>
    <li><strong>審査請求</strong>：却下の通知書を受け取った日の翌日から<strong>3か月以内</strong>に、<strong>都道府県知事あて</strong>に審査請求ができます（生活保護法第64条・行政不服審査法）。書式は自由で、費用はかかりません。福祉事務所ではなく都道府県・指定都市の担当に出します。</li>
