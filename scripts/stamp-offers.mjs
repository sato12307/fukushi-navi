...
const TARGETS = {
  ...
  'shogaisha-kojo-tax.html': {
    ...
    // ★2026-09-17 ②の末尾（買うボタンが8.4画面目）から、①早見表の直後（②の直前）へ。
    //   09-08 に「導入文が②の一文を引用しているのでこれ以上は上げられない」と書いたが、
    //   引用をやめて①の早見表を指す形に導入文を書き換えれば上げられる。制約は文面のほうにあった。
    before: /  <h2[^>]*>② 実際にいくら軽くなる？/,
    // 商品と読者が合っている唯一の pack の面。冒頭にも案内を出す。
    jumpBefore: /  <h2[^>]*>① 控除額はいくら？/,
    lead: '上の早見表の<strong>27万円・40万円・75万円</strong>を、自分の親御さんに当てはめて確かめ、認定書をもらって<strong>過去5年分をさかのぼって出す</strong>ところまでをまとめたものがあります。',
  },
  'juminzei-hikazei-check.html': {
    ...
    // ★2026-09-17 ②の判定結果の直後（買うボタンが7.7画面目）から、①早見表の直後（②の直前）へ。
    //   09-08 に「導入文が②の入力欄を引用しているので上げられない」と書いたが、
    //   引用をやめれば上げられる。制約は位置ではなく文面のほうにあった。
    before: /  <h2[^>]*>② あなたの世帯は？/,
    lead: '非課税の判定に出てくる「障害者・ひとり親・寡婦・未成年者」（合計所得135万円まで非課税）の<strong>障害者は、手帳を持っている人だけではありません</strong>。要介護認定を受けている親御さんは、市区町村の認定を受ければ障害者に当たることがあり、所得税・住民税の障害者控除も同時に使えます。申告していなかった分は<strong>過去5年分までさかのぼれます</strong>（制度の説明は<a href="shogaisha-kojo-tax.html">障害者控除の記事</a>に無料で置いています）。',
  },
  ...
  'seikatsuhogo-keisanki.html': {
    ...
    // ★2026-09-17 ②の計算結果の直後（買うボタンが8.6画面目）から、①の結論の直後（②の直前）へ。
    //   導入文は本文の語を引用していないので、そのまま動かせる。
    before: /  <h2[^>]*>② あなたの世帯でいくら？/,
